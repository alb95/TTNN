function B = toy_example_sequential_nystrom(A, X, Y, r, l)
B = cell(4,4);
AX = cell(3,4);
YAX = cell(3,4);
Q = cell(3,4);
...
YA{1,1} = permute(B{1,1}, [4,1,2,3]);
B{1,1} = tensorprod(B{1,1}, Y{1,2}, 1, 1);
YA{1,2} = permute(B{1,1}, [3,4,1,2]);
...
%---------
AX{1,2} = tensorprod(YA{1,1}, X{1,2}, [1,3,4], [1,2,3]);
...
%------
AX{1,3} = tensorprod(YA{1,2}, X{1,3}, [1,2], [1,2]);
...
YA{2,1} = tensorprod(A, Y{2,1}, [1,2], [1,2]);
YA{2,1} = permute(YA{2,1}, [5,1,2,3,4]);
...
%------
AX{2,2} = tensorprod(YA{2,1}, X{2,2}, [1,3,4,5], [1,2,3,4]);
YA{2,2} = tensorprod(YA{2,1}, Y{2,2}, 2, 1);
YA{2,2} = tensorprod(YA{2,2}, Y{1,2}, 2, 1); % added dummy node sketch
YA{2,2} = permute(YA{2,2}, [1,4,5,2,3]);
...
%------
AX{2,3} = tensorprod(YA{2,2}, X{2,3}, [1,2,3,5], [1,2,3,4]);
YA{2,3} = tensorprod(YA{2,2}, Y{2,3}, 4,1);
YA{2,3} = permute(YA{2,3}, [1,2,3,5,4]);
...
%------
AX{2,4} = tensorprod(YA{2,3}, X{2,4}, [1,2,3,4], [1,2,3,4]);
YA{2,4} = tensorprod(YA{2,3}, Y{2,4}, 5,1);
%YA{2,4} = permute(YA{2,4}, [1,2,3,4,5]); useless
...
YA{3,1} = tensorprod(A, Y{3,1}, 1, 1);
YA{3,1} = permute(YA{3,1}, [6,1,2,3,4,5]);
...
%------
AX{3,2} = tensorprod(YA{3,1}, X{3,2}, [1,3,4,5,6], [1,2,3,4,5]);
